...
export default class THTextInputForm extends Component {
    render() {
        const { label, keyboardType, value, security, meta: {touched, error, warning}, input: {onChange, ...restInput} } = this.props;
        return (
                <View style={THStyles.THIFmainContainer}>
                    <View style={THStyles.renderField}>
                        <Text style={THStyles.THIFlabelContainer}>{label}</Text>
                        <TextInput style={THStyles.THIFinputDefault} keyboardType={keyboardType} secureTextEntry={security} onChangeText={onChange} value={value} ></TextInput>
                    </View>
